...
export default function Home() {
  ...
  const getRiskBg = (score: number) => {
    ...
    if (score >= 4) return 'bg-amber-500/10 border-amber-500/20 text-amber-400'
    ...
    <main className="min-h-screen bg-[#080C14] text-white">
      ...
      <nav className="border-b border-white/5 px-4 sm:px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-white text-sm sm:text-base">ClearStatement</span>
        </div>
        ...
        {!result && (
          <>
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                { label: 'Faster than manual', value: '337x' },
                { label: 'Cheaper than Perfios', value: '10x' },
                { label: 'Bank formats', value: '12+' },
              ].map((stat) => (
                <div key={stat.label} className="bg-white/3 border border-white/8 rounded-xl sm:rounded-2xl p-3 sm:p-5 text-center">
            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl sm:rounded-3xl p-5 sm:p-8">
              <label className={`flex flex-col items-center justify-center w-full h-44 sm:h-52 border-2 border-dashed rounded-xl sm:rounded-2xl cursor-pointer transition-all duration-200 ${
                ...
                <div className="text-center px-4">
                  {file ? (
                    <>
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-emerald-500/15 flex items-center justify-center mx-auto mb-3">
                        <svg className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <p className="text-emerald-400 text-sm font-medium truncate max-w-[200px] sm:max-w-xs mx-auto">{file.name}</p>
                      <p className="text-gray-600 text-xs mt-1">Ready to analyze</p>
                    </>
                  ) : (
                    <>
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/5 flex items-center justify-center mx-auto mb-3">
              ...
              <button
                ...
                className={`w-full mt-4 sm:mt-5 font-semibold py-3.5 sm:py-4 rounded-xl transition-all duration-200 text-sm ${
                  loading
        ...
        {result && (
          <div>
            <div className="flex items-start sm:items-center justify-between mb-6 sm:mb-8 gap-3">
              ...
              <span className={`text-xs font-medium px-3 py-1.5 rounded-full border shrink-0 ${getRiskBg(result.risk_score)}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
              <div className="bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">Monthly Income</p>
              ...
              <div className="bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">Risk Score</p>
                <p className={`text-3xl sm:text-4xl font-bold ${getRiskColor(result.risk_score)}`}>
                  {result.risk_score}
                  <span className="text-gray-600 text-xl sm:text-2xl">/10</span>
                </p>
                <p className="text-gray-600 text-xs mt-2">{getRiskLabel(result.risk_score)}</p>
              </div>

              <div className="bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">Bounced Cheques</p>
                <p className={`text-3xl sm:text-4xl font-bold ${result.bounced_cheque_count === 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {result.bounced_cheque_count}
                </p>
                <p className="text-gray-600 text-xs mt-2">
                  {result.bounced_cheque_count === 0 ? 'No bounces detected' : 'Bounces found'}
                </p>
              </div>

              <div className="bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">Monthly EMI</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 sm:gap-4 mb-5">
              <div className="sm:col-span-2 bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">Avg Balance</p>
                <p className="text-2xl sm:text-3xl font-bold text-white">
                  ₹{result.average_balance?.toLocaleString('en-IN')}
                </p>
              ...
              <div className="sm:col-span-3 bg-white/3 border border-white/8 rounded-2xl p-5 sm:p-6">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2 sm:mb-3">AI Underwriter Summary</p>
            ...
            <button
              ...
              className="w-full bg-white/3 hover:bg-white/6 border border-white/8 text-gray-400 hover:text-white font-medium py-3.5 sm:py-4 rounded-xl transition-all text-sm"
            >
